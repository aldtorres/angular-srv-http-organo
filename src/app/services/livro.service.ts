  ...
  organizarLivrosPorGenero(): Observable<Map<string, Livro[]>>{
    return this.obterLivros().pipe(
      map((objLivros: Livro[]) =>{
        ...
        objLivros.forEach((livroItem: Livro) =>{
          const generoId = typeof livroItem.genero === 'string' ? livroItem.genero : livroItem.genero?.id;
          if(generoId){
            if(!livrosPorGenero.has(generoId)){
                livrosPorGenero.set(generoId, []);
            }
            livrosPorGenero.get(generoId)?.push(livroItem);
          }
